...
const PendingUserController: React.FC = () => {
  ...
  const getUserData = async (page: number, pageSize: number) => {
    setLoading(true);
    try {
      const userResponse = await getUsersAPI(page, pageSize, 'Pending');
  ...
  const handleActivate = async (row: any) => {
    setActiveId(row.id);
    try {
      const userResponse = await updateUserStatusAPI({userId: row.id, status : 'Active'});
      console.log('user Response:', userResponse);
      if(userResponse.success) {
        setActiveId('');
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: any) => (
        ...
        >
          {row.id === activeId ? 'Activating...' : 'Active' }
        </button>
      ),
      center: true,
    },
  ...
  return (
    <SidebarLayout>
      <TableComponent
        ...
        title={'New User - Approve Pending'}
      />
    </SidebarLayout>
